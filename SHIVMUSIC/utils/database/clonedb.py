...
# --- Clone thumbnail preference ---

async def get_clone_thumbnail_enabled(bot_id: Union[int, str]) -> bool:
    """Return whether a clone should send generated play thumbnails.

    The default is enabled to preserve the behaviour of existing clones.
    """
    try:
        data = await clone_custom_db.find_one({"bot_id": int(bot_id)})
        return True if not data else bool(data.get("thumbnail_enabled", True))
    except Exception as e:
        LOGGER.error(f"Error in get_clone_thumbnail_enabled: {e}")
        return True


async def set_clone_thumbnail_enabled(bot_id: Union[int, str], enabled: bool):
    try:
        await clone_custom_db.update_one(
            {"bot_id": int(bot_id)},
            {"$set": {"thumbnail_enabled": bool(enabled)}},
            upsert=True,
        )
    except Exception as e:
        LOGGER.error(f"Error in set_clone_thumbnail_enabled: {e}")
