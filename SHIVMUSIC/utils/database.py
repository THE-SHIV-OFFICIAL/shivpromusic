...
async def group_assistant(self, chat_id: int) -> int:
    ...
    if int(assis) == 1:
        ...
        return getattr(self, "two", None) or self.one
    elif int(assis) == 3:
        return getattr(self, "three", None) or self.one
    elif int(assis) == 4:
        return getattr(self, "four", None) or self.one
    elif int(assis) == 5:
        return getattr(self, "five", None) or self.one
...
async def is_thumbnail_enabled(chat_id: int) -> bool:
    """Return whether generated play thumbnails should be shown in a group."""
