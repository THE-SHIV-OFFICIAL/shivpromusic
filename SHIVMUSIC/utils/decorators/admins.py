...
def AdminRightsCheck(mystic):
    async def wrapper(client, message):
        ...
        if not is_non_admin and not await _is_authorized_user(
...
async def _is_authorized_user(chat_id, user_id):
    """Check the in-memory cache and the persisted /auth list."""
    ...
    try:
        ...
        return token in await get_authuser_names(chat_id)
    except Exception:
        return False
        ...
        if not is_non_admin:
            ...
            if not a.can_manage_video_chats:
                if CallbackQuery.from_user.id not in SUDOERS:
                    ...
                    _check = await get_authuser_names(CallbackQuery.message.chat.id)
