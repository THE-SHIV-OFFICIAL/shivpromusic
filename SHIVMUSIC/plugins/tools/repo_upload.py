...
def clean_stale_temps():
    """Delete leftover *.temp / partial downloads so failed runs don't pile up."""
    for pattern in ("*.temp", "*.zip.temp", "*_extract", "*_final"):
...
async def download_with_retry(replied, zip_path, attempts=3):
    """Download, retrying on '[Errno 24] Too many open files' after forcing
    garbage collection to release leaked file descriptors."""
    for attempt in range(1, attempts + 1):
        try:
            return await replied.download(file_name=zip_path)
        except OSError as e:
            if e.errno == errno.EMFILE and attempt < attempts:
                gc.collect()          # close unreferenced streams/sockets
                await asyncio.sleep(3 * attempt)
                safe_rm(zip_path + ".temp")
                continue
            raise
...
async def gitupload(client, message):
    ...
    # Unique paths per message — two parallel uploads can't clobber each other.
    uid = f"{message.id}"
    zip_path = os.path.join(TEMP_DIR, f"{uid}_{replied.document.file_name}")
    extract_root = os.path.join(TEMP_DIR, f"{uid}_{repo_name}_extract")
    final_path = os.path.join(TEMP_DIR, f"{uid}_{repo_name}_final")
    ...
    try:
        await download_with_retry(replied, zip_path)
        ...
        user = g.get_user()
        repo = user.create_repo(repo_name, private=is_private, description="🎉 sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ ᴜᴘʟᴏᴀᴅ ʙʏ :- THE SHIV 🌺", auto_init=False)
        ...
        if status_out.stdout.strip():
            run(["git", "commit", "-m", "THE SHIV !!"], cwd=final_path)
        else:
            run(["git", "commit", "--allow-empty", "-m", "THE SHIV !!"], cwd=final_path)
