...
def stream_markup2(_, chat_id, bot_username=None):
    bot_username = bot_username or getattr(app, "username", None) or "Telegram"
    s_map = get_style_map()
    buttons = [
        [add_me_button(bot_username, s_map[1])],
        [
...
def panel_markup_clone(_, vidid, chat_id, bot_username=None):
    s_map = get_style_map()
    bot_username = bot_username or getattr(app, "username", None) or "Telegram"
    buttons = [
        [
            create_btn(text="▷", callback_data=f"ADMIN Resume|{chat_id}", style=s_map[3]),
            create_btn(text="II", callback_data=f"ADMIN Pause|{chat_id}", style=s_map[3]),
            create_btn(text="‣‣I", callback_data=f"ADMIN Skip|{chat_id}", style=s_map[3]),
        ],
        [
            create_btn(text="◀ 20s", callback_data=f"ADMIN SeekBack|{chat_id}", style=s_map[4]),
            create_btn(text="20s ▶", callback_data=f"ADMIN SeekForward|{chat_id}", style=s_map[4]),
        ],
        [
            create_btn(text="🔁", callback_data=f"ADMIN Loop|{chat_id}", style=s_map[4]),
            create_btn(text="🔀", callback_data=f"ADMIN Shuffle|{chat_id}", style=s_map[4]),
        ],
        [add_me_button(bot_username, s_map[1])],
