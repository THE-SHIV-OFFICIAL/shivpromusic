...
@Client.on_message(filters.command(["thumbnail", "thumb", "setthumbnail"]))
async def clone_thumbnail_settings(client, message: Message):
    """Toggle generated play thumbnails for this clone bot."""
    if not await is_clone_owner(client, message):
        return
    ...
    if mode == "status":
        enabled = await get_clone_thumbnail_enabled(bot_id)
    else:
        enabled = mode == "on"
        await set_clone_thumbnail_enabled(bot_id, enabled)

    state = "ᴏɴ" if enabled else "ᴏғғ"
    return await message.reply_text(
        f"💎 <b>ᴄʟᴏɴᴇ ᴘʟᴀʏ ᴛʜᴜᴍʙɴᴀɪʟs ᴛᴜʀɴᴇᴅ {state}</b>\n"
