...
async def get_thumb(videoid, user_id, client):
    ...
    # Versioned cache prevents old channel/views cards from surviving updates.
    filename = f"cache/{videoid}_{bot_id}_premium_v3.png"
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(result["thumbnails"][0]["url"].split("?")[0]) as resp:
            async with aiofiles.open(f"cache/temp_{videoid}.jpg", mode="wb") as f:
                await f.write(await resp.read())

    with Image.open(f"cache/temp_{videoid}.jpg") as _src:
        bg = _src.convert("RGBA").resize((1920, 1080))
    ...
    try:
        ...
        br = ImageFont.truetype("SHIVMUSIC/assets/font2.ttf", 50)
    ...
    if u_photo:
        ...
        with Image.open(u_photo) as _up:
            u_img_blurred = _up.convert("RGBA").resize((450, 450)).filter(ImageFilter.GaussianBlur(6))
    ...
    draw.text((650, 285), "✨ NOW PLAYING", fill=(132, 224, 240), font=br)
    draw.text((650, 365), clear(title, 25), fill="white", font=f1)
    draw.text((650, 485), f"⏱️  Duration: {duration}", fill=(235, 235, 235), font=f2)
    draw.text((650, 555), "🎧  Premium Music Stream", fill=(200, 200, 200), font=f2)
