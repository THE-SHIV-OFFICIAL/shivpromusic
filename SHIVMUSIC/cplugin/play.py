...
async def send_clone_stream_card(client, chat_id, photo, caption, reply_markup):
    """Keep the same play card and controls when clone thumbnails are off."""
    if await get_clone_thumbnail_enabled(client.me.id) and photo:
    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                db[chat_id][-1]["client"] = client
                img = (
                    await get_thumb(vidid, user_id, client)
                    if await get_clone_thumbnail_enabled(client.me.id)
                    else None
                )
                
                button = panel_markup_clone(_, vidid, chat_id, bot_username)
                ...
                run = await send_clone_stream_card(
                    client,
                    original_chat_id,
                    img,
                    final_caption,
        ...
        if await is_active_chat(chat_id):
            ...
            run = await send_clone_stream_card(
                ...
                img,
                _["stream_1"].format(
                    f"https://t.me/{bot_username}?start=info_{vidid}",
                    title[:23],
                    duration_min,
                    user_name,
                ),
