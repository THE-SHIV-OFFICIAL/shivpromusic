...
from SHIVMUSIC.utils.database.clonedb import get_clone_thumbnail_enabled
...
async def _thumbnail_is_enabled(chat_client, chat_id):
    """Use group settings for the main bot and clone settings for clones."""
    try:
        bot = await chat_client.get_me()
        main_bot = getattr(app, "me", None) or await app.get_me()
        if bot.id != main_bot.id:
            return await get_clone_thumbnail_enabled(bot.id)
    except Exception:
        pass
    return await is_thumbnail_enabled(chat_id)


async def _send_stream_card(chat_client, chat_id, photo, caption, reply_markup):
    """Send a photo card when enabled, otherwise keep the same rich text card."""
    if await _thumbnail_is_enabled(chat_client, chat_id) and photo:
        return await chat_client.send_photo(
            chat_id=chat_id,
            photo=photo,
            caption=caption,
...
class Call:
    ...
    async def autoplay_start(self, chat_id: int, original_chat_id: int, seed_title: str, seed_vidid: str = None, client: PyTgCalls = None, bot_client = None) -> bool:
        ...
        try:
            thumbnail_enabled = await _thumbnail_is_enabled(chat_client, original_chat_id)
            img = (
                await get_thumb(track["vidid"], 0, chat_client)
                if thumbnail_enabled
                else None
            ) or (get_random_img(config.PLAYLIST_IMG_URL) if thumbnail_enabled else None)
            
            if is_clone:
                button = panel_markup_clone(_, track["vidid"], chat_id, bot_username)
            else:
                button = stream_markup(_, chat_id)
            
            run = await _send_stream_card(
                chat_client,
                original_chat_id,
                img,
                _["stream_1"].format(
            ...
            if "live_" in queued:
                ...
                try:
                    run = await _send_stream_card(
                        chat_client,
                        original_chat_id,
                        get_random_img(config.STREAM_IMG_URL),
                        _["stream_1"].format(
                ...
                thumbnail_enabled = await _thumbnail_is_enabled(chat_client, original_chat_id)
                img = (
                    await get_thumb(videoid, user_id, chat_client)
                    if thumbnail_enabled
                    else None
                ) or (get_random_img(config.PLAYLIST_IMG_URL) if thumbnail_enabled else None)
                
                if is_clone:
                    button = panel_markup_clone(_, videoid, chat_id, bot_username)
                else:
                    button = stream_markup(_, chat_id)
